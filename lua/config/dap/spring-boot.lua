...
local function get_spring_boot_runner(options)
  ...
  local debug_param = ""
  if debug then
    debug_param = ' -Dspring-boot.run.jvmArguments="-Xmx' ..
    memory .. ' -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=y,address=5005" '
  else
...
vim.keymap.set("n", "<C-F11>", function() M.run_spring_boot_quick(true) end, { desc = "Spring Boot: Debug rápido" })
